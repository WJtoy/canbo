...
@Slf4j
@Component
public class B2BExceptionCollectMQSender implements RabbitTemplate.ConfirmCallback {
        ...
        public void send(MQB2BExceptionMessage.B2BExceptionMessage message) {
            try {
                kklRabbitRetryTemplate.execute((RetryCallback<Object, Exception>) context -> {
                    ...
                    kklRabbitTemplate.convertAndSend(
                            B2BMQConstant.MQ_B2B_EXCEPTION_COLLECT,
                            B2BMQConstant.MQ_B2B_EXCEPTION_COLLECT,
                            message.toByteArray(),
                            new CorrelationData());
                    return null;
                    ...
                    sysLogService.insert(1L,new Gson().toJson(msg),
                            "短信队列发送失败：" + throwable.getLocalizedMessage() +"；错误原因：" + throwable.getCause().toString(),
                            "短信队列发送失败", B2BMQConstant.MQ_B2BCENTER_B2BWORKCARDQTYDAILY, null);
                    log.error("短信队列发送失败", throwable.getLocalizedMessage(), msg);
                    return null;
                });
            }catch (Exception e) {
                e.printStackTrace();
